...
def main():
    ...
    if args.emit_init:
        ...
        print(f"/* Generated by tools/fcp_decode.py --emit-init from {src}. Do not edit.")
        print(" * Device bring-up replayed at probe; struct clarett_init_step is in clarett.h. */")
        print("static const u8 clarett_init_blob[] = {")
        for i in range(0, len(blob), 12):
            print("\t" + " ".join(f"0x{b:02x}," for b in blob[i:i + 12]))
        print("};")
        print("static const struct clarett_init_step clarett_init_seq[] = {")
